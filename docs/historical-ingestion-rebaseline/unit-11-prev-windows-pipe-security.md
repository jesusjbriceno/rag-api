...
- **L8 — E3 and E4 are closed by an authorized evidence exception, not by execution.** No foreign-user connect
  attempt and no remote/non-local connect attempt has ever been run against this boundary on any host. The two
  facts acceptance letter (b) names — that a foreign user is denied and that a remote client is denied — therefore
  rest on the descriptor read in E2 (owner-only, DACL protected, one allow ACE, no broad principals) and on
  `PIPE_REJECT_REMOTE_CLIENTS` being passed explicitly to `CreateNamedPipeW`, **not on a refused connection**.
  The maintainer authorized this exception on 2026-09-22 to unblock the dependent chain (Unit 11 → Unit 12 →
  Unit 13 → Unit 14), with the residual risk recorded here rather than implied away.

  **Residual risk.** A descriptor read does show what the object manager will enforce, but it is not a refused
  connection, and this slice already contains a precedent for the difference: run 1's peer check looked correct in
  the source and denied every legitimate peer the moment it ran. Nothing here proves
  `PIPE_REJECT_REMOTE_CLIENTS` either — the flag is present in the creation call and no remote client has ever
  been refused by it. A defect that made the DACL or the remote flag ineffective would stay invisible until a
  foreign or remote client is actually attempted. Retiring the exception still costs what it always did: one
  elevated session for E3 (a local test account with a known password, and `seclogon` started, which `runas`
  requires) and one more Windows host on the same network for E4. Linux nodes on the same LAN cannot substitute,
  because opening an arbitrary named-pipe name needs an SMB client library they do not carry.

## 7. Sign-off

| Field | Value |
| --- | --- |
| Rows passed / failed | **Run 1:** E2 pass · E5 pass · E6 pass · E1 FAIL · E7 FAIL · E3/E4 not executed → 3 pass, 2 fail, 2 not executed. **Run 2 (after the BF-1 fix):** E1 pass · E2 pass · E5 pass · E6 pass · E7 pass · **E3 and E4 closed by an authorized evidence exception, not executed** (§6 L8) |
| Blocking findings | **None.** BF-1 was blocking and is fixed and re-verified (§5.4). **E3 and E4 were never executed and are closed by a user-authorized evidence exception** (§6 L8): no foreign-user and no remote connect attempt has ever run, so those two facts rest on E2's descriptor and on the remote-rejection flag, not on a refused connection. That is a recorded exception with its residual risk stated, **not a verification**, and the exception exists only because the maintainer authorized it to unblock the dependent chain. |
...
| Notes | Run 1: commit `1b89a6ea538dd590935605327b022914f04e8a5d`, Engine DLL SHA-256 `77af3bf826590bef6c1de96744175976095841c93e104771aa5d9011155864f5`. Run 2: the same commit **plus the BF-1 fix**, Engine DLL SHA-256 `cdf0fee4036317c25e214c35be366a2b11a8c3c802b886c1d106121744e0a89c`; both are now committed and published on `feat/historical-ingestion-windows-probe` (`bc5b523` the fix, `105b8b7` this record's first version, `41ae3aa` the operator runbook, `1304bf0` the redaction of machine-identifying values). Branch `feat/historical-ingestion-windows-probe`; separate clone at `C:\src\rag-api-windows-evidence`; the operator's working clone was not modified. Every server was stopped with a console control event (no forced kill), all engine and probe processes are confirmed gone, no `rag-historical-loader-v1*` pipe remains and the test locks are released. **11.prev-e is closed by an authorized evidence exception, not by a complete acceptance run:** E1, E2, E5, E6 and E7 pass, while **E3 and E4 were never executed** and are carried by the exception recorded in §6 L8. Any later reader must treat those two letters as unverified until a foreign user and a remote client are actually refused. |
