...
public sealed class HandleConfinedCopier
{
    ...
    public HandleConfinedCopier(
        IEnumerable<string> roots,
        string snapshotDirectory,
        TimeProvider? clock = null,
        SourceOpener? openSource = null)
    {
    ...
    public async Task<string> CopyAsync(string sourcePath, CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 1; attempt <= MaxRetries; attempt++)
        {
            try
            {
                return await CopyOnceAsync(fullPath, cancellationToken).ConfigureAwait(false);
            }
            catch (IOException ex) when (IsSharingViolation(ex))
            {
                ...
                {
                    await Task.Delay(Backoff(attempt), _clock, cancellationToken).ConfigureAwait(false);
                }
            }
        }

        throw sharingViolation
            ?? new IOException($"Snapshot failed for '{fullPath}' after {MaxRetries} sharing retries.");
    }
    ...
    private void EnsureSnapshotDirectory()
    {
        ...
        if (ConfinedSnapshotService.IsLink(new DirectoryInfo(_snapshotDirectory)))
        {
            throw new IOException($"Snapshot directory is a reparse point: '{_snapshotDirectory}'.");
        }
    }

    private void EnsureConfined(string path)
    {
        if (!IsUnderAnyRoot(path))
        {
            throw new IOException($"Source escapes the configured roots: '{path}'.");
        }
        if (ConfinedSnapshotService.IsLink(new FileInfo(path)))
        {
            ...
            if (target is null || !IsUnderAnyRoot(target))
            {
                throw new IOException($"Source is a reparse point escaping the configured roots: '{path}'.");
            }
        }
    }

    private static (long Length, DateTime LastWriteTimeUtc) CaptureIdentity(string path)
    {
        var info = new FileInfo(path);
        return (info.Length, info.LastWriteTimeUtc);
    }

    private static void EnsureUnchanged(string path, (long Length, DateTime LastWriteTimeUtc) identity)
    {
        var current = CaptureIdentity(path);
        if (current.Length != identity.Length || current.LastWriteTimeUtc != identity.LastWriteTimeUtc)
        {
            throw new IOException($"Source changed during snapshot (drift detected): '{path}'.");
        }
    }
    ...
    private bool IsUnderAnyRoot(string path)
    {
        ...
        {
            // Windows paths are case-insensitive; the companion targets win-x64.
